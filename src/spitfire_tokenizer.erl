...
tokenize([$?, $\\, H | T], Line, Column, Scope, Tokens) ->
  ...
  Token = {char, {Line, Column, Line, Column + 3, [$?, $\\, H]}, Char},
...
tokenize([$?, Char | T], Line, Column, Scope, Tokens) ->
  ...
  Token = {char, {Line, Column, Line, Column + 2, [$?, Char]}, Char},
...
tokenize(".:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 2, Scope, [{kw_identifier, {Line, Column, Line, Column + 2, nil}, '.'} | Tokens]);

tokenize("<<>>:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 5, Scope, [{kw_identifier, {Line, Column, Line, Column + 5, nil}, '<<>>'} | Tokens]);
tokenize("%{}:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 4, Scope, [{kw_identifier, {Line, Column, Line, Column + 4, nil}, '%{}'} | Tokens]);
tokenize("%:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 2, Scope, [{kw_identifier, {Line, Column, Line, Column + 2, nil}, '%'} | Tokens]);
tokenize("&:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 2, Scope, [{kw_identifier, {Line, Column, Line, Column + 2, nil}, '&'} | Tokens]);
tokenize("{}:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 3, Scope, [{kw_identifier, {Line, Column, Line, Column + 3, nil}, '{}'} | Tokens]);
tokenize("..//:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 5, Scope, [{kw_identifier, {Line, Column, Line, Column + 5, nil}, '..//'} | Tokens]);

tokenize(":<<>>" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 5, Scope, [{atom, {Line, Column, Line, Column + 5, nil}, '<<>>'} | Tokens]);
tokenize(":%{}" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 4, Scope, [{atom, {Line, Column, Line, Column + 4, nil}, '%{}'} | Tokens]);
tokenize(":%" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 2, Scope, [{atom, {Line, Column, Line, Column + 2, nil}, '%'} | Tokens]);
tokenize(":{}" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 3, Scope, [{atom, {Line, Column, Line, Column + 3, nil}, '{}'} | Tokens]);
tokenize(":..//" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 5, Scope, [{atom, {Line, Column, Line, Column + 5, nil}, '..//'} | Tokens]);
...
tokenize([$:, T1, T2, T3 | Rest], Line, Column, Scope, Tokens) when
  ...
  Token = {atom, {Line, Column, Line, Column + 4, nil}, list_to_atom([T1, T2, T3])},
...
tokenize([$:, $:, $: | Rest], Line, Column, Scope, Tokens) ->
  ...
  Token = {atom, {Line, Column, Line, Column + 3, nil}, '::'},
...
tokenize([$:, T1, T2 | Rest], Line, Column, Scope, Tokens) when
  ...
  Token = {atom, {Line, Column, Line, Column + 3, nil}, list_to_atom([T1, T2])},
...
tokenize([$:, T | Rest], Line, Column, Scope, Tokens) when
  ...
  Token = {atom, {Line, Column, Line, Column + 2, nil}, list_to_atom([T])},
  tokenize(Rest, Line, Column + 2, Scope, [Token | Tokens]);

% ## Stand-alone tokens

tokenize("=>" ++ Rest, Line, Column, Scope, Tokens) ->
  Token = {assoc_op, {Line, Column, Line, Column + 2, previous_was_eol(Tokens)}, '=>'},
...
tokenize("..//" ++ Rest = String, Line, Column, Scope, Tokens) ->
  case strip_horizontal_space(Rest, 0) of
    {[$/ | _] = Remaining, Extra} ->
      Token = {identifier, {Line, Column, Line, Column + 4 + Extra, nil}, '..//'},
...
tokenize([$, | Rest], Line, Column, Scope, Tokens) ->
  Token = {',', {Line, Column, Line, Column + 1, 0}},
  tokenize(Rest, Line, Column + 1, Scope, [Token | Tokens]);

tokenize([$<, $< | Rest], Line, Column, Scope, Tokens) ->
  Token = {'<<', {Line, Column, Line, Column + 2, nil}},
  handle_terminator(Rest, Line, Column + 2, Scope, Token, Tokens);

tokenize([$>, $> | Rest], Line, Column, Scope, Tokens) ->
  Token = {'>>', {Line, Column, Line, Column + 2, previous_was_eol(Tokens)}},
  handle_terminator(Rest, Line, Column + 2, Scope, Token, Tokens);
...
tokenize([T | Rest], Line, Column, Scope, Tokens) when T =:= $(; T =:= ${; T =:= $[ ->
  Token = {list_to_atom([T]), {Line, Column, Line, Column + 1, nil}},
  handle_terminator(Rest, Line, Column + 1, Scope, Token, Tokens);

tokenize([T | Rest], Line, Column, Scope, Tokens) when T =:= $); T =:= $}; T =:= $] ->
  Token = {list_to_atom([T]), {Line, Column, Line, Column + 1, previous_was_eol(Tokens)}},
  handle_terminator(Rest, Line, Column + 1, Scope, Token, Tokens);

% ## Two Token Operators
tokenize([T1, T2 | Rest], Line, Column, Scope, Tokens) when ?ternary_op(T1, T2) ->
  Op = list_to_atom([T1, T2]),
  Token = {ternary_op, {Line, Column, Line, Column + 2, previous_was_eol(Tokens)}, Op},
  tokenize(Rest, Line, Column + 2, Scope, add_token_with_eol(Token, Tokens));
...
tokenize([$& | Rest], Line, Column, Scope, Tokens) ->
  ...
  Token = {Kind, {Line, Column, Line, Column + 1, nil}, '&'},
  tokenize(Rest, Line, Column + 1, Scope, [Token | Tokens]);
      ...
      case unescape_tokens(Parts, Line, Column, NewScope) of
        {ok, [Part]} when is_binary(Part) ->
          case unsafe_to_atom(Part, Line, Column, Scope) of
            {ok, Atom} ->
              Token = {atom_quoted, {Line, Column, NewLine, NewColumn, H}, Atom},
        ...
        {ok, Unescaped} ->
          ...
          Token = {Key, {Line, Column, NewLine, NewColumn, H}, Unescaped},
          tokenize(Rest, NewLine, NewColumn, NewScope, [Token | Tokens]);
...
tokenize([$: | String] = Original, Line, Column, Scope, Tokens) ->
  case tokenize_identifier(String, Line, Column, Scope, false) of
    {_Kind, Unencoded, Atom, Rest, Length, Ascii, _Special} ->
      ...
      Token = {atom, {Line, Column, Line, Column + 1 + Length, Unencoded}, Atom},
...
tokenize([H | T], Line, Column, Scope, Tokens) when ?is_digit(H) ->
  case tokenize_number(T, [H], 1, false) of
    ...
    {Rest, Number, Original, Length} when is_integer(Number) ->
      Token = {int, {Line, Column, Line, Column + Length, Number}, Original},
      tokenize(Rest, Line, Column + Length, Scope, [Token | Tokens]);
    {Rest, Number, Original, Length} ->
      Token = {flt, {Line, Column, Line, Column + Length, Number}, Original},
      tokenize(Rest, Line, Column + Length, Scope, [Token | Tokens])
  end;
...
tokenize(";" ++ Rest, Line, Column, Scope, []) ->
  tokenize(Rest, Line, Column + 1, Scope, [{';', {Line, Column, Line, Column + 1, 0}}]);

tokenize(";" ++ Rest, Line, Column, Scope, [Top | _] = Tokens) when element(1, Top) /= ';' ->
  tokenize(Rest, Line, Column + 1, Scope, [{';', {Line, Column, Line, Column + 1, 0}} | Tokens]);
...
tokenize([$%, ${ | T], Line, Column, Scope, Tokens) ->
  Token = {'{', {Line, Column, Line, Column + 1, nil}},
  handle_terminator(T, Line, Column + 2, Scope, Token, [{'%{}', {Line, Column, Line, Column + 2, nil}} | Tokens]);

tokenize([$% | T], Line, Column, Scope, Tokens) ->
  tokenize(T, Line, Column + 1, Scope, [{'%', {Line, Column, Line, Column + 1, nil}} | Tokens]);

tokenize([$. | T], Line, Column, Scope, Tokens) ->
  tokenize_dot(T, Line, Column + 1, {Line, Column, Line, Column + 1, nil}, Scope, Tokens);
    ...
    {Kind, Unencoded, Atom, Rest, Length, Ascii, Special} ->
      ...
      case Rest of
        [$: | T] when ?is_space(hd(T)) ->
          Token = {kw_identifier, {Line, Column, Line, Column + Length, Unencoded}, Atom},
        ...
        _ when Kind == identifier ->
          ...
          Token = check_call_identifier(Line, Column, Unencoded, Atom, Rest, Length),
...
handle_heredocs(T, Line, Column, H, Scope, Tokens) ->
  case extract_heredoc_with_interpolation(Line, Column, Scope, true, T, H) of
    {ok, NewLine, NewColumn, Parts, Rest, NewScope} ->
      case unescape_tokens(Parts, Line, Column, NewScope) of
        {ok, Unescaped} ->
          Token = {heredoc_type(H), {Line, Column, NewLine, NewColumn, nil}, NewColumn - 4, Unescaped},
          tokenize(Rest, NewLine, NewColumn, NewScope, [Token | Tokens]);
...
handle_strings(T, Line, Column, H, Scope, Tokens) ->
  case spitfire_interpolation:extract(Line, Column, Scope, true, T, H) of
      ...
      case unescape_tokens(Parts, Line, Column, NewScope) of
        {ok, [Part]} when is_binary(Part) ->
          case unsafe_to_atom(Part, Line, Column - 1, Scope) of
            {ok, Atom} ->
              Token = {kw_identifier, {Line, Column - 1, NewLine, NewColumn + 1, H}, Atom},
        ...
        {ok, Unescaped} ->
          ...
          Token = {Key, {Line, Column - 1, NewLine, NewColumn + 1, H}, Unescaped},
          tokenize(Rest, NewLine, NewColumn + 1, NewScope, [Token | Tokens]);
    ...
    {NewLine, NewColumn, Parts, Rest, InterScope} ->
      ...
      case unescape_tokens(Parts, Line, Column, NewScope) of
        {ok, Unescaped} ->
          Token = {string_type(H), {Line, Column - 1, NewLine, NewColumn, nil}, Unescaped},
          tokenize(Rest, NewLine, NewColumn, NewScope, [Token | Tokens]);
...
handle_op(Rest, Line, Column, Kind, Length, Op, Scope, Tokens) ->
  case strip_horizontal_space(Rest, 0) of
    ...
    {Remaining, Extra} ->
      ...
      Token = {Kind, {Line, Column, Line, Column + Length + Extra, previous_was_eol(Tokens)}, Op},
  ...
  case spitfire_interpolation:extract(Line, Column + 1, Scope, true, T, H) of
    {NewLine, NewColumn, [Part], Rest, InterScope} when is_list(Part) ->
      ...
      case unsafe_to_atom(UnescapedPart, Line, Column, NewScope) of
        {ok, Atom} ->
          Token = case Rest of
            [$( | _] -> {paren_identifier, {Line, Column, NewLine, NewColumn, H}, Atom};
            [$[ | _] -> {bracket_identifier, {Line, Column, NewLine, NewColumn, H}, Atom};
            _ -> {identifier, {Line, Column, NewLine, NewColumn, H}, Atom}
          end,
...
handle_call_identifier(Rest, Line, Column, DotInfo, Length, UnencodedOp, Scope, Tokens) ->
  Token = check_call_identifier(Line, Column, UnencodedOp, list_to_atom(UnencodedOp), Rest, Length),
  TokensSoFar = add_token_with_eol({'.', DotInfo}, Tokens),
...
handle_space_sensitive_tokens([Sign, NotMarker | T], Line, Column, Scope, [{identifier, _, _} = H | Tokens]) when
  ...
  Rest = [NotMarker | T],
  DualOpToken = {dual_op, {Line, Column, Line, Column + 1, nil}, list_to_atom([Sign])},
...
eol(_Line, _Column, [{',', {Line, Column, _, _, Count}} | Tokens]) ->
  [{',', {Line, Column, Line, Column + 1, Count + 1}} | Tokens];
eol(_Line, _Column, [{';', {Line, Column, _, _, Count}} | Tokens]) ->
  [{';', {Line, Column, Line, Column + 1, Count + 1}} | Tokens];
eol(_Line, _Column, [{eol, {Line, Column, _, _, Count}} | Tokens]) ->
  [{eol, {Line, Column, Line, Column + 1, Count + 1}} | Tokens];
eol(Line, Column, Tokens) ->
  [{eol, {Line, Column, Line, Column + 1, 1}} | Tokens].
...
extract_heredoc_indent({_, {EndLine, _, _, _, _}, _} = Part, {Warned, _Line}, _Indent) ->
  {Part, {Warned, EndLine}}.
...
reset_eol([{eol, {Line, Column, _, _, _}} | Rest]) -> [{eol, {Line, Column, Line, Column + 1, 0}} | Rest];
reset_eol(Rest) -> Rest.
...
tokenize_alias(Rest, Line, Column, Unencoded, Atom, Length, Ascii, Special, Scope, Tokens) ->
  if
    ...
    true ->
      AliasesToken = {alias, {Line, Column, Line, Column + Length, Unencoded}, Atom},
      tokenize(Rest, Line, Column + Length, Scope, [AliasesToken | Tokens])
  end.

%% Check if it is a call identifier (paren | bracket | do)

check_call_identifier(Line, Column, Info, Atom, [$( | _], Length) ->
  {paren_identifier, {Line, Column, Line, Column + Length, Info}, Atom};
check_call_identifier(Line, Column, Info, Atom, [$[ | _], Length) ->
  {bracket_identifier, {Line, Column, Line, Column + Length, Info}, Atom};
check_call_identifier(Line, Column, Info, Atom, _Rest, Length) ->
  {identifier, {Line, Column, Line, Column + Length, Info}, Atom}.

add_token_with_eol({unary_op, _, _} = Left, T) -> [Left | T];
add_token_with_eol(Left, [{eol, _} | T]) -> [Left | T];
add_token_with_eol(Left, T) -> [Left | T].

previous_was_eol([{',', {_, _, _, _, Count}} | _]) when Count > 0 -> Count;
previous_was_eol([{';', {_, _, _, _, Count}} | _]) when Count > 0 -> Count;
previous_was_eol([{eol, {_, _, _, _, Count}} | _]) when Count > 0 -> Count;
...
handle_terminator(Rest, _, _, Scope, {'(', {Line, Column, _, _, _}}, [{alias, _, Alias} | Tokens]) when is_atom(Alias) ->
  Reason =
    io_lib:format(
...
check_terminator({'end', {EndLine, _, _, _, _}}, [{'do', _, Indentation} | Terminators], Scope) ->
  NewScope =
...
check_terminator({'end', {Line, Column, _, _, _}}, [], #spitfire_tokenizer{mismatch_hints=Hints}) ->
  Suffix =
...
tokenize_keyword(token, Rest, Line, Column, Atom, Length, Scope, Tokens) ->
  Token = {Atom, {Line, Column, Line, Column + Length, nil}},
  tokenize(Rest, Line, Column + Length, Scope, [Token | Tokens]);

tokenize_keyword(block, Rest, Line, Column, Atom, Length, Scope, Tokens) ->
  Token = {block_identifier, {Line, Column, Line, Column + Length, nil}, Atom},
  tokenize(Rest, Line, Column + Length, Scope, [Token | Tokens]);
...
tokenize_keyword(Kind, Rest, Line, Column, Atom, Length, Scope, Tokens) ->
  NewTokens =
    case strip_horizontal_space(Rest, 0) of
      {[$/ | _], _} ->
        [{identifier, {Line, Column, Line, Column + Length, nil}, Atom} | Tokens];
      ...
      _ ->
        case {Kind, Tokens} of
          ...
          {_, _} ->
            add_token_with_eol({Kind, {Line, Column, Line, Column + Length, previous_was_eol(Tokens)}, Atom}, Tokens)
        end
    end,
...
add_sigil_token(SigilName, Line, Column, NewLine, NewColumn, Parts, Rest, Scope, Tokens, Indentation, Delimiter) ->
  ...
  case MaybeEncoded of
    {ok, Atom} ->
      ...
      Token = {sigil, {Line, TokenColumn, NewLine, NewColumnWithModifiers, nil}, Atom, Parts, Modifiers, Indentation, Delimiter},
...
tokenize_keyword_terminator(DoLine, DoColumn, do, [{identifier, {Line, Column, _, _, Meta}, Atom} | T]) ->
  {ok, add_token_with_eol({do, {DoLine, DoColumn, DoLine, DoColumn + 2, nil}},
                          [{do_identifier, {Line, Column, Line, Column + length(atom_to_list(Atom)), Meta}, Atom} | T])};
...
tokenize_keyword_terminator(Line, Column, do, Tokens) ->
  case is_valid_do(Tokens) of
    true  -> {ok, add_token_with_eol({do, {Line, Column, Line, Column + 2, nil}}, Tokens)};
    false -> {error, invalid_do_error("unexpected reserved word: "), "do"}
  end;
tokenize_keyword_terminator(Line, Column, Atom, Tokens) ->
  {ok, [{Atom, {Line, Column, Line, Column + length(atom_to_list(Atom)), nil}} | Tokens]}.
...
add_cursor(Line, Column, prune_and_cursor, Terminators, Tokens) ->
  ...
  CursorTokens = [
    {')', {Line, Column + 11, Line, Column + 12, nil}},
    {'(', {Line, Column + 10, Line, Column + 11, nil}},
    {paren_identifier, {Line, Column, Line, Column + 10, nil}, '__cursor__'}
    | PrunedTokens
  ],
