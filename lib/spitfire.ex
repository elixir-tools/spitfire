defmodule Spitfire do
  ...
  defp parse_stab_expression(parser) do
    trace "parse_stab_expression", trace_meta(parser) do
      ...
      {exprs, parser} =
        while2 is_nil(Map.get(parser, :stab_state)) and
                 peek_token(parser) not in [:end, :eof, :")", :block_identifier, :->] <-
                 parser do
          parser = parser |> next_token() |> eat_eoe()
          ...
          if peek_token(parser) == :";" do
            ...
            if Map.get(parser, :stab_state) do
              {:filter, {nil, parser}}
            else
              {ast, parser}
            end
    ...
    trace "parse_do_block", trace_meta(parser) do
      ...
      {exprs, {_, parser}} =
        while2 peek_token_eat_eoe(parser) not in [:end, :eof] <- {type, parser} do
          {exprs, parser} =
            while2 peek_token_eat_eoe(parser) not in [:end, :block_identifier, :eof] or stab_state_set?(parser) <- parser do
              {ast, parser} =
                case Map.get(parser, :stab_state) do
                  %{ast: lhs} ->
                    parser =
